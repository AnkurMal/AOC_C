...
#define foreach(item, var)                                          \
    for (size_t keep = 1, count = 0; keep && count != (var).len;    \
         keep = !keep, count++)                                     \
        for (typeof((var).arr[0])*(item) = &(var).arr[count]; keep; \
             keep = !keep)

#define foreach_enumerate(item, index, var)                          \
    for (size_t _keep = 1, index = 0; _keep && index != (var).len;   \
         _keep = !_keep, index++)                                    \
        for (typeof((var).arr[0])*(item) = &(var).arr[index]; _keep; \
             _keep = !_keep)

#define String(str) string_new(str)

#define string_append(str_ptr, append_ptr)          \
    _Generic((append_ptr),                          \
        ...
        default: _report_error_internal)(str_ptr, append_ptr)

#define string_insert(str_ptr, insert_ptr, index)   \
    _Generic((insert_ptr),                          \
        ...
        default: _report_error_internal)(str_ptr, insert_ptr, index)

#define string_parse(str_ptr, type)                                 \
    _Generic((type)0,                                               \
        ...
        default: _report_error_internal)(str_ptr)

#define string_find(str_ptr, pattern)             \
    _Generic((pattern),                           \
        ...
        default: _report_error_internal)(str_ptr, pattern)

#define string_remove(str_ptr, pattern)             \
    _Generic((pattern),                             \
...
#define string_equals(str1_ptr, str2_ptr)          \
    _Generic((str2_ptr),                           \
        ...
        default: _report_error_internal)(str1_ptr, str2_ptr)

#define split_remove_all(split_ptr, pattern)        \
    _Generic((pattern),                             \
        ...
        default: _report_error_internal)(split_ptr, pattern)

#define split_contains(split_ptr, str_ptr)            \
    _Generic((str_ptr),                               \
        ...
        default: _report_error_internal)(split_ptr, str_ptr)
...
void _insert_str_internal_string(String* str, const String* insert,
                                 int64_t index);
