...
void _insert_str_internal_char(String* str, const char* insert, int64_t index) {
    ...
    __assert((size_t)index <= str->len,
             "Inserting at index %lld, but the length is %zd.", index,
             str->len);
...
void _remove_str_internal_char(String* str, const char* pat) {
    ...
    if (index != -1) {
        memmove(&str->arr[index], &str->arr[index + len],
                str->len - (index + len));
        str->len -= len;
    }
}
...
void _remove_str_internal_string(String* str, const String* pat) {
    ...
    if (index != -1) {
        memmove(&str->arr[index], &str->arr[index + pat->len],
                str->len - (index + pat->len));
        str->len -= pat->len;
    }
}
...
void string_print_debug(String* str) {
    printf("String { len: %zu, capacity: %zu, arr: \"", str->len,
           str->capacity);
    string_print(str);
    printf("\" }");
}
...
char string_at(String* str, int64_t index) {
    ...
    __assert((size_t)index < str->len,
             "Accessing at index %lld, but the length is %zd.", index,
             str->len);
    ...
    if (del_len == 0) {
        ...
        while (current <= str->len - del_len) {
            if (memcmp(&str->arr[current], delimeter, del_len) == 0) {
                split_push(&spl,
                           string_with_len(&str->arr[track], current - track));
                current += del_len;
                track = current;
            } else
                current++;
        ...
        if (track <= str->len)
            split_push(&spl,
                       string_with_len(&str->arr[track], str->len - track));
    }
...
void string_map(String* str, void (*func)(String* str)) { func(str); }
...
bool string_is_empty(String* str) { return str->len == 0; }

void string_trim_right(String* str) {
    if (str->len != 0)
        while (str->len > 0 && isspace(str->arr[str->len - 1])) str->len--;
}

void string_trim_left(String* str) {
    if (str->len != 0) {
        size_t start = 0;
        while (start < str->len && isspace(str->arr[start])) start++;
...
StringSplit string_lines(String* str) { return string_split(str, NEWLINE); }
...
void split_print_debug(StringSplit* split) {
    printf("StringSplit { len: %zu, capacity: %zu, arr: ", split->len,
           split->capacity);
    split_print(split);
    printf(" }");
}
...
String split_at(StringSplit* split, int64_t index) {
    __assert(index >= 0, "Accessing at index %lld is not allowed.", index);
    __assert((size_t)index < split->len,
             "Accessing at index %lld, but the length is %zd.", index,
             split->len);
